...
async def sudo(Data, payload, *text):
    if payload.get('Author') in admins: await payload['user'].add_roles([payload['refs']['roles']['Moderator'],])
async def sudont(Data, payload, *text):
    if payload.get('Author') in admins: await payload['user'].remove_roles([payload['refs']['roles']['Moderator'],])
