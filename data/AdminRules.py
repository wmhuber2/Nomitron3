...
async def sudo(Data, payload, *text):
    if payload.get('Author') in admins: await payload['refs']['players'][payload.get('Author ID')].add_roles([payload['refs']['roles']['Moderator'],])
async def sudont(Data, payload, *text):
    if payload.get('Author') in admins: await payload['refs']['players'][payload.get('Author ID')].remove_roles([payload['refs']['roles']['Moderator'],])
