...
async def sudo(Data, payload, *text):
    if payload.get('Author') in admins: await payload['raw'].author.add_roles([payload['refs']['roles']['Moderator'],])
async def sudont(Data, payload, *text):
    if payload.get('Author') in admins: await payload['raw'].author.remove_roles([payload['refs']['roles']['Moderator'],])
